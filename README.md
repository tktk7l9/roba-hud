...
- **キーマップ表示** — `zmk-config-roBa` の `config/roBa.keymap` / `config/roBa.json` を直接パースし、実配置（親指キーの回転含む）でレイヤー別に描画。**roBa がこのMacのアクティブな BT プロファイルの間だけ表示**（他プロファイルに切り替えて 45 秒操作がないと自動的に隠れ、このMacに戻すと即座に前面へ戻る。roBa は非アクティブなホストとも BLE 接続を維持し続けるため、接続/切断ではなく直近の HID 操作の有無で判定）。全 Spaces / フルスクリーン上でも表示
